param (
    ...
    [Parameter(Mandatory)][string]$NCUsername,
    [Parameter(Mandatory)][string]$NCapi
)
...
function Invoke-NCAPI {
    ...
    try {
        ...
        for ($i=1; $i -le 5; $i++) {
            ...
            $response = Invoke-RestMethod $apiBase -Body $body -Method $Method -UseBasicParsing -EA Stop
...
if ($command -eq "create") {
    Add-DnsTxtNamecheap $RecordName $TxtValue $NCUsername $NCapi
} elseif ($command -eq "remove") {
    Remove-DnsTxtNamecheap $RecordName $TxtValue $NCUsername $NCapi
}