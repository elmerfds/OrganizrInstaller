param (
    ...
    [Parameter(Mandatory)][string]$CFemail,
    [Parameter(Mandatory)][string]$CFapi
)
...
function Add-DnsTxtCloudflare {
    ...
    $response = Invoke-RestMethod "$apiRoot/$zoneID/dns_records?type=TXT&name=$RecordName&content=$TxtValue" `
        -Headers $authHeader -ContentType 'application/json' -UseBasicParsing #@script:UseBasic
...
function Find-CFZone {
    ...
    for ($i=1; $i -lt ($pieces.Count-1); $i++) {
        ...
        $response = Invoke-RestMethod "$apiRoot/?name=$zoneTest" -Headers $authHeader -UseBasicParsing #@script:UseBasic 
...
if ($command -eq "create") {
    Add-DnsTxtCloudflare $RecordName $TxtValue $CFemail $CFapi
} elseif ($command -eq "remove") {
    Remove-DnsTxtCloudflare $RecordName $TxtValue $CFemail $CFapi
}