param (
    ...
    [Parameter(Mandatory)][string]$GDKey,
    [Parameter(Mandatory)][string]$GDSecret
)
...
function Add-DnsTxtGoDaddy {
    ...
    try {
        $recs = Invoke-RestMethod "$apiRoot/$zone/records/TXT/$recShort" `
            -Headers $headers -UseBasicParsing -EA Stop
    } catch { throw }
...
function Find-GDZone {
    ...
    try {
        $zones = (Invoke-RestMethod $apiRoot -Headers $headers -UseBasicParsing -EA Stop) `
...
if ($command -eq "create") {
    Add-DnsTxtGoDaddy $RecordName $TxtValue $GDKey $GDSecret
} elseif ($command -eq "remove") {
    Remove-DnsTxtGoDaddy $RecordName $TxtValue $GDKey $GDSecret
}